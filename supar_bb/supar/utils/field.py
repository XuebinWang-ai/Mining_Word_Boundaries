...
class BiLabelField(Field):
    r"""
    encode bigram word-seg labels, such as bb be ...
    """
    ...
    def transform(self, sequences):
        ...
        # _ V
        # b e
        n_tags = len(self.vocab)
        flag_b_m_e_s = sequences[0][0] in {'b', 'm', 'e', 's'} # whole dataset
        results = []
        for sequence in sequences:
            ...
            for (i, label) in enumerate(sequence):
                if flag_b_m_e_s:
                    label = label + (sequence[i+1] if i < len(sequence) - 1 else 's')
                    label_list = [0] * n_tags
                    label_list[ self.vocab[label] ] = 1
                elif '_' == label:
                    label_list = [1] * n_tags
                else:
                    assert 'V' == label, f'label: {label}'
                    label_list = [0] * n_tags
                    for label in self.specials_w_boundary:
                        label_list[ self.vocab[label] ] = 1
                results[-1].append(label_list)
            # print(results[-1])
        sequences = [torch.tensor(seq).bool() for seq in results]
        return sequences
