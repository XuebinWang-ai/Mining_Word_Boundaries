...
class Coarse2FineCharCRFDependency(CharCRFDependency):
    ...
    """
    @torch.enable_grad()
    def forward(self, scores, mask, c_span_mask=None, i_span_mask=None, target=None, partial=False, marginal=None, normalizer='token'):
        r"""
        Args:
            scores (~torch.Tensor): ``[batch_size, seq_len, seq_len]``.
                Scores of all possible dependent-head pairs.
        ...
        # print('logZ', logZ, 'score', score, 'norm', norm)
        loss = (logZ - score) / norm
    ...
    @torch.enable_grad()
    def labeling(self, scores, s_labels, mask, target, c_span_mask=None, i_span_mask=None, normalizer='token'):
        """merge unlabeled and labeled loss by adding label (log) probability to unlabeled scores"""
        logZ = self.inside(scores.requires_grad_(), mask, LogSemiring)
        scores = self.mask(scores + s_labels, target)
        # the second inside process is necessary if using partial annotation
        score = self.inside(scores.requires_grad_(),
                            mask, LogSemiring, c_span_mask, i_span_mask)
        if normalizer == 'token':
            norm = mask.sum()
        elif normalizer == 'sentence':
            norm = mask.size(0)
    ...
    @torch.enable_grad()
    def marginalize(self, scores, mask, target=None, c_span_mask=None, i_span_mask=None):
        """Get marginal probability for elements in scores."""
        # marginals are used for decoding, and can be computed by combining the inside pass and autograd mechanism
        scores = self.mask(scores, target) if target is not None else scores
        logZ = self.inside(scores.requires_grad_(), mask,
                           LogSemiring, c_span_mask, i_span_mask)
        marginals, = autograd.grad(logZ, scores)
        return marginals

    @torch.enable_grad()
    def decode(self, scores, mask, target=None, c_span_mask=None, i_span_mask=None):
        """
        Args:
        ...
        scores = self.mask(scores, target) if target is not None else scores
        logZ = self.inside(scores.requires_grad_(), mask,
                           MaxSemiring, c_span_mask, i_span_mask)
        marginals, = autograd.grad(logZ, scores)

        return marginals

    def inside(self, s_arc, mask, semiring, c_span_mask=None, i_span_mask=None):
        """
        Args:
        ...
        inter_combine_rules = mask.new_tensor([0, 1, 0, 1])
        ...
        for w in range(1, seq_len):
            ...
            # ilr = C(i->r) + C(j->r+1)
            # TODO: [n, w, batch_size, n_state] -> [n_state, n, w, batch_size]
            # [n, w, batch_size, n_state] -> [batch_size, n, w, n_state]
            ir = stripe(s_c, n, w).permute(2, 0, 1, 3)  # C(i->r)
            il = stripe(s_c, n, w, (w, 1)).permute(2, 0, 1, 3)  # C(j->r+1)
            # [batch_size, n, w, n_state*n_state]
            # TODO: ir.unsqueeze(0) + il.unsqueeze(1)
            ...
            # NOTE: it is no necessary to mask out illegal inter-spans link operations,
            # since inter-arcs have already been masked out these illegal operations
