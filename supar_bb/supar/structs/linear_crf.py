...
class LinearCRFWordSegmentation(nn.Module):
    ...
    def __init__(self, masked_crf=True):
        ...
        self.trans_mask = nn.Parameter(
            torch.Tensor(self.n_tags, self.n_tags), requires_grad=False)
    ...
    def forward(self, s_tag, tags, mask):
        ...
        logZ = self.crf(s_tag, mask, None, False)
        logZ_constrained = self.crf(s_tag, mask, tags, True)        
        # assert False
        # print('logZ', logZ)
        # print('logZ_constrained', logZ_constrained)
        loss = (logZ - logZ_constrained) / batch_size
        return loss
    ...
    def get_scores(self, s_tag, tags, mask):
        ...
        mask_wo_ending_position = mask.clone().detach()
        mask_wo_ending_position[end_pos, batch_idx] = 0
        # [seq_len, batch_size]
        ...
        score = scores.masked_select(mask_wo_ending_position).sum()
        # the start position
        score += self.start_scores[tags[0] // 4].sum()
        # [batch_size]

        # the end position
        score += self.end_scores[tags.gather(0, end_pos.unsqueeze(0)) // 4].sum()
        # purpose of // 4: bb bm ... 16 bigram-tags, to obtain the first tag id

        return score

    @torch.enable_grad()
    def get_marg_probs(self, s_tag, mask):
        """
        Args:
        ...
        """
        logZ = self.crf(s_tag.requires_grad_(), mask)
        # gradients, = autograd.grad(logZ, s_tag, allow_unused=True)
        gradients, = autograd.grad(logZ, s_tag)
        end_pos = mask.sum(dim=-1)
        marg_probs = []
        # pdb.set_trace()
        for (i, grad) in enumerate(gradients):
            marg_probs.append([])
            for j in range(end_pos[i]): # char
                marg_probs[-1].append(grad[j].tolist()) # probs of 16 tags
        return marg_probs

    def crf(self, s_tag, mask, tags=None, forbid_illegal_trans=True):
        """
        Args:
            s_tag (Tensor): emission scores of tags, [batch_size, seq_len, n_tags]
            mask (Tensor): mask of the input, [batch_size, seq_len]
        ...
        batch_size, seq_len, n_tags = s_tag.shape
        if tags is not None:
            assert tags.shape == s_tag.shape
            # False positions 全部赋予 -inf 的分数，就不会转移到该 position    
            s_tag = s_tag.masked_fill(~tags, MIN)

        # [seq_len, batch_size, n_tags]
        s_tag = s_tag.permute(1, 0, 2)
        # [seq_len, batch_size, 16] -> [seq_len, batch_size, 4, 4]
        s_bitag_score = s_tag.view(seq_len, batch_size, 4, 4)  

        # chart
        s = s_tag.new_full((seq_len, batch_size, 4), MIN)
        # start position
        # [batch_size, n_tags]
        s[0] = self.start_scores # the 0-th position holds the first char

        if forbid_illegal_trans:
            s[0] += self.start_mask
            s_bitag_score = s_bitag_score + self.trans_mask

        for i in range(1, seq_len):
            # [batch_size, n_tags, n_tags]
            scores = s[i-1].unsqueeze(-1) + s_bitag_score[i-1] # seq_len=5 i=4 i-1=3
        ...
        if forbid_illegal_trans:
            scores += self.end_mask
        return scores.logsumexp(dim=-1).sum()

    @torch.enable_grad()
    def viterbi(self, s_tag, mask, tags):
        batch_size, seq_len, n_tags = s_tag.shape
        ...
        # [seq_len, batch_size, 16] -> [seq_len, batch_size, 4, 4]
        s_bitag_score = s_tag.view(seq_len, batch_size, 4, 4) + self.trans_mask

        s = s_tag.new_full((seq_len, batch_size, 4), 0)

        # start position
        # [batch_size, n_tags]
        s[0] = self.start_scores + self.start_mask

        for i in range(1, seq_len):
            # [batch_size, n_tags, n_tags]
            scores = s[i-1].unsqueeze(-1) + s_bitag_score[i-1]
        ...
        scores = s[end_pos, batch_idx]
        # end position can only be `E` and `S`
        scores += self.end_scores + self.end_mask

        # [batch_size]
        scores, _ = torch.max(scores, dim=-1)
        # [seq_len, batch_size, n_tags]
        gradients, = autograd.grad(scores.sum(), s_tag)
        # print('gradients', gradients.shape)
        # print('lens', mask.sum(dim=-1))
        return [i.nonzero()[:, 1].tolist() for i in gradients.permute(1, 0, 2)]

        # return [i.nonzero()[:, 1].tolist() for i in gradients.permute(1, 0, 2)]
        # assert [i.nonzero()[:, 1].tolist() for i in gradients.permute(1, 0, 2)] == [torch.argmax(i, dim=-1).tolist() for i in gradients.permute(1, 0, 2)]
        res = []
        for i, l in zip(gradients.permute(1, 0, 2), end_pos):
            print(i.nonzero()[:, 1].tolist(), len(i.nonzero()[:, 1].tolist()))
            print(torch.argmax(i, dim=-1).tolist()[: l+1], l+1)
            res.append(torch.argmax(i, dim=-1).tolist()[: l+1])
            assert torch.argmax(i, dim=-1).tolist()[: l+1] == i.nonzero()[:, 1].tolist()
            # import sys
            # sys.exit(0)
        # x = [i.nonzero()[:, 1].tolist() for i in gradients.permute(1, 0, 2)]
