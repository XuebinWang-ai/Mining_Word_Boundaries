...
class ExplicitCharCRFDependencyModel(CharCRFDependencyModel):
    ...
    def decode(self, s_arc, s_rel, mask, nul_mask=None, orientation='leftward', tree=False, proj=False):
        r"""
        Args:
            s_arc (~torch.Tensor): ``[batch_size, seq_len, seq_len]``.
                Scores of all possible arcs.
...
class Coarse2FineCharCRFDependencyModel(LatentCharCRFDependencyModel):
    ...
    def loss(self, s_arc, s_rel, arcs, rels, mask, c_span_mask=None, i_span_mask=None):
        ...
        if self.args.label_loss == 'crf':
            ...
            loss = self.crf.labeling(s_arc, s_rel, mask, target=arcs, c_span_mask=c_span_mask, i_span_mask=i_span_mask, normalizer=self.args.struct_normalizer)
            return loss

        arc_loss, marginals = self.crf(s_arc, mask, target=arcs, partial=True, normalizer=self.args.struct_normalizer, c_span_mask=c_span_mask, i_span_mask=i_span_mask, marginal=self.args.marginal)
    ...
    def decode(self, s_arc, s_rel, mask, target=None, c_span_mask=None, i_span_mask=None):
        ...
        s_arc[:, :, 0, 0] = MIN
        # [batch_size, seq_len, seq_len, n_states]
        arc_preds = self.crf.decode(s_arc, mask, target=target, c_span_mask=c_span_mask, i_span_mask=i_span_mask)
...
class SplitCoarse2FineCharCRFDependencyModel(Coarse2FineCharCRFDependencyModel):
    ...
    def forward(self, chars, feats=None):
        ...
        if self.args.shared_arc_info == 'repr' and self.args.use_orth_loss and self.training:
            return s_arc, s_rel, orth_loss

        return s_arc, s_rel, None
