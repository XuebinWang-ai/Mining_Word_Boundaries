...
class Parser(object):
    ...
    def train(self, train, train2, dev, test, buckets=32, batch_size=5000, update_steps=1,
        ...
        # batch_size=5000
        # if the gpu memory is not enough, we can set update_steps=2; then the real batch_size=2500
        train.build(args.batch_size//args.update_steps, args.buckets, True, dist.is_initialized())
        # IMPORTANT: shuffle the data before each epoch
        dev.build(args.batch_size, args.buckets)
        test.build(args.batch_size, args.buckets)
        logger.info(f"\n{'train:':6} {train}\n{'dev:':6} {dev}\n{'test:':6} {test}\n")
        if args.use_train2:
            train2 = Dataset(self.transform, args.train2, **args)
            train2.build(args.batch_size//args.update_steps, args.buckets, True, dist.is_initialized())
            # if len(train2) > len(train):
            #     train2_batch_size = train2.chars_num // len(train.loader) - 1
            #     train2.build(train2_batch_size, args.buckets, True, dist.is_initialized())
            # if len(train) > len(train2):
            #     train_batch_size = train.chars_num // len(train2.loader) - 1
            #     train.build(train_batch_size, args.buckets, True, dist.is_initialized())
        ...
        if args.encoder == 'lstm':
            ...
            if args.use_train2:
                # steps = min((len(train2.loader)//len(train.loader)+1) * len(train.loader), 
                #              len(train2.loader)) * epochs // args.update_steps
                # steps = (len(train.loader)+len(train2.loader)) * epochs // args.update_steps   # 部分数据
                steps = len(train.loader) * epochs // args.update_steps   # 部分数据
            else:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if args.use_train2:
                print(f'using train2.loader')
                self._train(train.loader, train2.loader)
            else:
                print(f'using train1.loader only')
                self._train(train.loader, None)
            # import sys
            # sys.exit(1)
